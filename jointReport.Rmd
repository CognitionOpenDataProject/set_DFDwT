...
## compare agency mean and sd (rounded) vs. reported values
# agent = 1
compareValues(obtainedValue = agencyEffortCombined_M, reportedValue = -322) # match
compareValues(obtainedValue = agencyEffortCombined_SD, reportedValue =  236) # match

# non-agent = 2
compareValues(obtainedValue = noagencyEffortCombined_M, reportedValue = -56) # match
compareValues(obtainedValue = noagencyEffortCombined_SD, reportedValue = 265) # match
...
## compare effort mean and sd (rounded) vs. reported values
# high effort
compareValues(obtainedValue = highEffortAgencyCombined_M, reportedValue = -170) # match
compareValues(obtainedValue = highEffortAgencyCombined_SD, reportedValue = 228) # match

# low effort
compareValues(obtainedValue = loweffortAgencyCombined_M,reportedValue = -208) # match
compareValues(obtainedValue = loweffortAgencyCombined_SD,reportedValue = 230) # match
```

Summary: In our first attempt we encountered some errors. However, after discussing the issue with the authors we discovered we were aggregating the data incorrectly. When aggregation was done per the author's instructions, we could successfully reproduce all values.
...
compareValues(reportedValue = 1.59, obtainedValue = ttest2$statistic) # minor error
...
This reproducibility check appears to be a success in terms of the main findings and statistics being reproduced. We encountered some difficulties reproducing standard deviations at first. However, after discussing the issue with the authors, we discovered we were aggregating the data differently. When we followed the author's instructions, our reproducibility check was a complete success. 
